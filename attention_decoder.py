...
def attention_decoder(decoder_inputs, auxiliary_input, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
  """
  Args:
    decoder_inputs: A list of 2D Tensors [batch_size x input_size].
    auxiliary_input: 2D Tensor [batch_size x aux_input_size] 
  ...
  with variable_scope.variable_scope("attention_decoder") as scope:
    ...
    for i, inp in enumerate(decoder_inputs):
      ...
      # Merge input , conversation context, previous attentions into one vector x of the same size as inp
      input_size = inp.get_shape().with_rank(2)[1]
      if input_size.value is None:
        raise ValueError("Could not infer input size from input: %s" % inp.name)
      x = linear([inp] + [context_vector] + [auxiliary_input], input_size, True)
      ...
      if pointer_gen:
        with tf.variable_scope('calculate_pgen'):
          p_gen = linear([context_vector, state.c, state.h, x, auxiliary_input], 1, True) # Tensor shape (batch_size, 1)
      ...
      with variable_scope.variable_scope("AttnOutputProjection"):
        output = linear([cell_output] + [context_vector] + [auxiliary_input], cell.output_size, True)
